function allMets=evaluateTrackingMota(seqmap,resDir,dataDir)
...
%     stI.Xi(find(stI.Xi(:)))=-1;
    ...
    [mets, mInf, addInfo]=CLEAR_MOT_HUN(gtInfoSingle(seqCnt).gtInfo,stI);
    
    allMets(mcnt).mets2d(seqCnt).name=seqName;
    allMets(mcnt).mets2d(seqCnt).m=mets;
    allMets(mcnt).mets2d(seqCnt).modak=addInfo.MODAK;
    allMets(mcnt).mets2d(seqCnt).motak=addInfo.MOTAK;
