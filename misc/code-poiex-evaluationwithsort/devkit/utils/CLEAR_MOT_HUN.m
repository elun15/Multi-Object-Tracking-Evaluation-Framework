...
gForModa = g;
gForModa(g==0) = 1; % this is necessary because there are divisions by zero otherwise
MODA = mean(1 - (m + fp)./gForModa);
MODAK = 1 - (m + fp)./gForModa;
...
metrics=[recall, precision, FAR, Ngt, MT, PT, ML, falsepositives, missed, idswitches, FRA, MOTA, MOTP, MODA];

additionalInfo.alltracked=alltracked;
additionalInfo.allfalsepos=allfalsepos;
additionalInfo.MODAK = MODAK;
additionalInfo.MOTAK = MOTAK;
end 
