    ...
    * 'ESC' : quit
    ...
    def rectangle(self, x, y, w, h, label=None):
        ...
        if label is not None:
            ...
            text_size = cv2.getTextSize( label, cv2.FONT_HERSHEY_PLAIN, 2, self.thickness)
            center = pt1[0] + 5, pt1[1] + 5 + text_size[0][1]
            pt2 = pt1[0] + 20 + text_size[0][0], pt1[1] + 20 + text_size[0][1]
            cv2.rectangle(self.image, pt1, pt2, self._color, -1)
            cv2.putText(self.image, label, center, cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), self.thickness)
